...
//			
	...
	public String reportOrgReached(String treeOrgId,String reportType,String year,String orgId,String positionId,String employeeId,String currency,Model model){
		...
		int saleSupportReportScale = 0;//售前支持反馈率
		int overdueNoReturnPrototypeSum = 0;//逾期未收回样机
		if(reportType != null && "ORG".equals(reportType.toUpperCase())){
			...
			bizoppReportScale = reportService.getBizoppReportScale("ORG",orgId);
			bizoppOrderReportScale = reportService.getBizoppOrderReportScale("ORG",orgId);
			reportOrderScale = reportService.getReportOrderScale("ORG",orgId);
		...
		model.addAttribute("actualListOne",JSON.toJSONString(contactVeri.toListOne()));
	...
	public String selectLovTree(String pickerId,String posId,String groupId,String leafFlag,String rootId,String opType,Model model){
	...
	private String contructTree(Condition condition, HttpServletRequest request,int optType){
		...
		list = reportService.getLovList(rootId,groupId,leafFlag,parentId,posId);
	 ...
	 */
	@NoRight
	@RequestMapping("/overdueNoReturnPrototypeList")
	public String overdueNoReturnPrototypeList(String reportType, String orgIdOrEmployeeId,String currency,String year,Model model){
		model.addAttribute("reportType", reportType);
		model.addAttribute("orgIdOrEmployeeId", orgIdOrEmployeeId);
		model.addAttribute("currency", currency);
		model.addAttribute("year", year);
		return forward("overdueNoReturnPrototype");
	}
	
	@NoRight
	@ResponseBody
	@RequestMapping("/overdueNoReturnPrototype")
	public String overdueNoReturnPrototype(PageCondition condition,String reportType, String orgIdOrEmployeeId,String currency,String year, HttpServletRequest request){
		ActionUtil.requestToCondition(condition, request);
		IPage p = reportService.getOverdueNoReturnPrototype(condition,reportType,orgIdOrEmployeeId,currency,year);
		return sendSuccessMessage(p);
	}
	 ...
	 */
	@NoRight
	@RequestMapping("/veriDetail")
	public String veriDetail(String reportType, String orgIdOrEmployeeId,int month,String year,String currency,Model model){
		String monthStr = "";
		if(month < 10){
			monthStr = "0"+month;
		}else {
		...
		model.addAttribute("year", year);
		model.addAttribute("currency", currency);
		return forward("veriDetail");
	}
	
	@NoRight
	@ResponseBody
	@RequestMapping("/veriDetailList")
	public String veriDetailList(PageCondition condition,String reportType, String orgIdOrEmployeeId,String month,String year,String currency,HttpServletRequest request){
