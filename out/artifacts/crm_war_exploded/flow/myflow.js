(function(b) {
	...
	a.rect = function(p, m) {
		...
		if(p.type == 'task'){
			t.dblclick(function(){ showTaskAttr(p.type,'t') });
			e.dblclick(function(){ showTaskAttr(p.type,'e') });
			n.dblclick(function(){ showTaskAttr(p.type,'n') });
			f.dblclick(function(){ showTaskAttr(p.type,'f') });
		}
		else if(p.type == 'multiTask'){
			t.dblclick(function(){ showTaskAttr(p.type) });
			e.dblclick(function(){ showTaskAttr(p.type) });
			n.dblclick(function(){ showTaskAttr(p.type) });
			f.dblclick(function(){ showTaskAttr(p.type) });
		}
		else if(p.type == 'decision'){
			t.dblclick(function(){ showDecisionAttr() });
			e.dblclick(function(){ showDecisionAttr() });
			n.dblclick(function(){ showDecisionAttr() });
			f.dblclick(function(){ showDecisionAttr() });
		}else if(p.type = 'notice'){
			t.dblclick(function(){ showTaskAttr('notice') });
			e.dblclick(function(){ showTaskAttr('notice') });
			n.dblclick(function(){ showTaskAttr('notice') });
			f.dblclick(function(){ showTaskAttr('notice') });
		}
		t.drag(function(r, o) {
			A(r, o)
		},
		...
		this.toJson = function() {
			var r = "{id:'"+this.getId()+"' , type:'" + E.type + "',text:'" + f.attr("text") + "', x:" + Math.round(t.attr("x")) + ", y:" + Math.round(t.attr("y")) + ", width:" + Math.round(t.attr("width")) + ", height:" + Math.round(t.attr("height")) + ", props:{";
	...
	a.path = function(q, n, u, e) {
		...
		this.toJson = function() {
			var r = "{from:'" + k.getId() + "',to:'" + s.getId() + "', dots:" + x.toJson() + ",text:'" + f.attr("text") + "',textPos:{x:" + Math.round(h.x) + ",y:" + Math.round(h.y) + "}, props:{";
