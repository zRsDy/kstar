package com.ibm.kstar.action.bizopp.conflict;
...
public class BizConflictAction extends BaseAction {
	...
	public String page(PageCondition condition, HttpServletRequest request) throws Exception {
		...
		condition.getFilterObject().addOrCondition("conflictStatus", "=", "10");// 已提交申诉
		condition.getFilterObject().addOrCondition("conflictStatus", "=", "30");// 冲突待确认
	...
	public String rejected(String id,String remark) {
		BusinessOpportunity businessOpportunity = bizoppService.getBizOppEntity(id);
		if(!"15".equals(businessOpportunity.getStatus())){
		...
		bizoppService.rejected(businessOpportunity,remark);
		return sendSuccessMessage();
	}

	@NoRight
	@ResponseBody
	@RequestMapping(value = "/btnx", method = RequestMethod.POST)
	public String btnx(String id,String remark) {
		BusinessOpportunity businessOpportunity = bizoppService.getBizOppEntity(id);
//		processStatusService.updateProcessStatus("BusinessOpportunity", id, "status", "30");
//		processStatusService.updateProcessStatus("BusinessOpportunity", id, "conflictStatus", "20");
		//启动流程
		if(businessOpportunity != null) {
			bizoppService.startBtnxProcess(businessOpportunity, getUserObject(),remark);
		}else{
			throw new AnneException("无商机id");
		}
		return sendSuccessMessage();
	}
	...
	public String publish(String id,
		...
		List<BusinessOpportunity> bizIds = new ArrayList<>();
		// 发布冲突生成冲突编号
...
bizoppService.publishMail(bizIds);

		return sendSuccessMessage();
	}

	@RequestMapping(value = "/appeal")
	public String conflictAppeal(String bizOppId,Model model){
		if (StringUtil.isEmpty(bizOppId)) {
            throw new AnneException("商机Id不能为空");
        }
        BusinessOpportunity businessOpportunity = this.bizoppService.get(bizOppId);
        if (businessOpportunity == null) {
            throw new AnneException("商机不存在");
        }
        model.addAttribute("entity", businessOpportunity);
		return forward("conflictAppeal");
	}

	@NoRight
	@ResponseBody
	@RequestMapping(value = "/appeal",method = RequestMethod.POST)
	public String conflictAppeal(BusinessOpportunity opportunity) {
		if (StringUtil.isEmpty(opportunity.getId())) {
			throw new AnneException("商机Id不能为空");
		}
        if (!Arrays.asList("Y", "N").contains(opportunity.getConflictAppeal())) {
            throw new AnneException("商机冲突未选择是");
        }

        if (StringUtil.isEmpty(opportunity.getConflictStatus())) {
            throw new AnneException("商机状态不能为空");
        }

        if (StringUtil.isEmpty(opportunity.getConflictAppealRemark())) {
            throw new AnneException("备注不能为空");
        }

        this.bizoppService.updateForConflictAppeal(opportunity);
		return sendSuccessMessage();
	}
}
