...
public class DeliveryAction extends BaseAction {
	...
	public String checkCanDelete(HttpServletRequest request, HttpServletResponse response){
		...
		String ret = "No" ;
		String isErpDeliveryFlag = deliveryService.checkIsErpDelivery(id,op);
		String isHasReceipt = deliveryService.checkHasReceipt(id,op);
		String isErpStatus = deliveryService.checkErpStatus(id);
		if(!ret.equals(isErpDeliveryFlag)) {
			return sendSuccessMessage(isErpDeliveryFlag);
		}else if(!ret.equals(isHasReceipt)){
			return sendSuccessMessage(isHasReceipt);
		}else if(!ret.equals(isErpStatus)){
			return sendSuccessMessage(isErpStatus);
		}else {
