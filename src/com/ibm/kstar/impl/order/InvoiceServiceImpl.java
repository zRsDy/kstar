...
public class InvoiceServiceImpl implements IInvoiceService {
	...
	public void validateCanInvoice(String businessKey){
		List<Object> args = new ArrayList<Object>();
		StringBuffer hql =  new StringBuffer("select detail "
				+ "from InvoiceDetail detail , OrderLinesView ol, OrderHeader o , LovMember lov ");
		hql.append(" where ");
		hql.append(" detail.orderLineId = ol.id ");
		hql.append(" and ol.orderId = o.id ");
		hql.append(" and detail.invoiceType = '02' ");
		hql.append(" and detail.invoiceId = ? ");
		args.add(businessKey);
		hql.append(" and o.salesmanCenter = lov.id ");
		hql.append(" and (ol.erpStatusDes != '已发货' and lov.code = '10')");// or lov.code = '20' ) ");
		...
		String sql = "select ol.c_is_erp_delivery from crm_t_invoice_detail dl,CRM_V_ORDER_LINES ol "
				+ "where dl.c_order_line_id=ol.c_pid and dl.c_invoice_id='"+businessKey+"' and "
						+ "ol.c_is_erp_delivery!='Yes' and dl.c_invoice_type='02' and rownum=1";
		*/
		if(baseDao.exist(hql.toString(),args.toArray())){
			throw new AnneException("该开票申请存在未发货的订单行,不能提交");
		}
		
		checkInvoice(businessKey);
	}
