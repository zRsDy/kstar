...
public class DeliveryServiceImpl extends MessageAdapter<String>  implements IDeliveryService {
	...
	public IPage getDeliveryLinesByHQL(PageCondition condition) throws Exception {
		...
		hql.append(" and header.status = ? ");
		args.add(IConstants.ORDER_CONTROL_STATUS_30);
		...
		if( linesList != null){
			for (Map<Object, Object> map : linesList) {
				...
				if(deliveryLines != null){
					...
					if(!StringUtil.isNullOrEmpty(deliveryHeader.getId())) {
		...
		if(StringUtil.isNotEmpty(id)){
			if("delivery".equals(op)){
				...
				if(deliveryReceipt != null){
					...
					if(!StringUtil.isNullOrEmpty(deliveryReceipt.getLogisticsNo())) {
						return "HasLogisticsNo";
					}
				}
			}
		}
...
//			}
	...
	@Override
	public void updateDeliveryLineQtyByID(String id, double deliveryQty,UserObject userObject,double quantity){
		DeliveryLines deliveryLines = baseDao.get(DeliveryLines.class, id);
		if(deliveryLines != null){
			deliveryLines.setDeliveryQty(deliveryQty);
			deliveryLines.setResidualQty(deliveryLines.getResidualQty() - deliveryQty);
