...
//        hql.append(" h.erpOrderCode");
    ...
    public void splitLineByDelivery(final OrderLines orderLine, final DeliveryLines deliveryLine, final double quantity, final UserObject userObject) {
        ...
        MethodLoggerStep loggerStep = MethodLoggerStep.setup(methodLogService, "com.ibm.kstar.impl.order.OrderServiceImpl.splitLineByDelivery", deliveryLine.getDeliveryCode());

        loggerStep.doLog("com.ibm.kstar.impl.order.OrderServiceImpl.checkBeforeSplitLineForDelivery", new Object[]{orderHeader, orderLine, deliveryLine, quantity}, new MethodLoggerStep.IBusinessAction() {
            @Override
            public void doAction(List<Object> logedObject) {
                checkBeforeSplitLineForDelivery(orderHeader, orderLine, deliveryLine, quantity, userObject);
            }
        });
        loggerStep.doLog("com.ibm.kstar.impl.order.OrderServiceImpl.splitLine", new Object[]{orderHeader, orderLine, quantity}, new MethodLoggerStep.IBusinessAction() {
            @Override
            public void doAction(List<Object> logedObject) {
                splitLine(orderHeader, orderLine, quantity, userObject);
            }
    ...
    private void checkBeforeSplitLineForDelivery(@NotNull OrderHeader orderHeader,@NotNull OrderLines orderLines, DeliveryLines deliveryLine, double quantity, UserObject userObject){
        ...
        if (IConstants.ORDER_LINE_STATUS_PICKED.equals(orderLines.getStatus())) {
            ...
            // 更新发货单行数量
            deliveryService.updateDeliveryLineQtyByID(deliveryLine.getId(), deliveryQty, userObject,quantity);
        }
