...
<#macro picker_report_tree id callback groupId rootId='' posId='' opType='' button="选择" defaultSelect='[]' leafFlag='' >
	<button class="btn btn-info" id='${id}' type="button" > <i class="icon-undo bigger-110"></i> ${button} </button>
	<script type="text/javascript">
		var selectedDatas_${id} = ${defaultSelect};
		$('#${id}').click(function(){
			parent.dialog2("选择","${ctx}/report/selectLovTree.html?leafFlag=${leafFlag!}&posId=${posId}&pickerId=${id}&groupId=${groupId}&rootId=${rootId!}&opType=${opType}",selectedDatas_${id},window);
		})
		function ${id}_callback(datas){
			var callback = ${callback};
			callback(datas);
