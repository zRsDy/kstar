		...
		for (; inputIndex < inputLength; inputIndex++) {
			for (key in input[inputIndex]) {
				...
				if (input[inputIndex].hasOwnProperty(key)
						&& value !== undefined) {
					target[key] = $.isPlainObject(value) ? $.widget.extend({},
	...
	$.Widget.prototype = {
		...
		_on : function(element, handlers) {
			...
			$.each(handlers, function(event, handler) {
				...
				if (selector) {
					instance.widget().delegate(selector, eventName,
							handlerProxy);
				} else {
...
(function($, undefined) {
	...
	var uiDialogClasses = "ui-dialog ui-widget ui-widget-content ui-corner-all zs-dialog-border", sizeRelatedOptions = {
		buttons : true,
		height : true,
		maxHeight : true,
		maxWidth : true,
					...
					"ui.dialog",
					{
						...
						_create : function() {
							...
							uiDialogTitlebarCloseText = (this.uiDialogTitlebarCloseText = $("<span>"))
									.addClass(
											"ui-icon ui-icon-closethick floatcloseicon")
	...
	(function() {
		...
		function getElementStyles() {
			var style = this.ownerDocument.defaultView ? this.ownerDocument.defaultView
					...
					: this.currentStyle, newStyle = {}, key, camelCase, len;
...
(function($, undefined) {
	...
	$.effects.effect.size = function(o, done) {
		...
		mode = $.effects.setMode(el, o.mode || "effect"), restore = o.restore
				...
				|| [ "middle", "center" ], original, baseline, factor, position = el
				.css("position");
	...
	$
			.widget(
					...
					$.ui.mouse,
					{
						...
						_create : function() {
							var i, o = this.options, existingHandles = this.element
								...
								for ( var j = this.items.length - 1; j >= 0; j--) {
									if (!$
											.contains(
													...
													this.items[j].item[0]))
										continue;
									var cur = this.containers[innermostIndex].floating ? this.items[j].item
											.offset().left
											: this.items[j].item.offset().top;
									if (Math.abs(cur - base) < dist) {
										dist = Math.abs(cur - base);
										itemWithLeastDistance = this.items[j];
										this.direction = (cur - base > 0) ? 'down'
												: 'up';
									}
								}
	...
	$
			.widget(
					"ui.tabs",
					{
						...
						refresh : function() {
							var next, options = this.options, lis = this.tablist
									.children(":has(a[href])");
				...
				return $.extend({}, ajaxOptions, {
					error : function(xhr, s, e) {
						try {
							...
							// $(a.hash)
							ajaxOptions.error(xhr, s, ui.tab.closest("li")
									.index(), ui.tab[0]);
						} catch (e) {
						}
					}
...
(function($) {
	...
	$.widget("ui.tooltip", {
		...
		open : function(event) {
			var target = $(event ? event.target : this.element).closest(
					this.options.items);
		...
		close : function(event, force) {
			...
			//
			// we have to check the event type because tabbing out of the
			// document
